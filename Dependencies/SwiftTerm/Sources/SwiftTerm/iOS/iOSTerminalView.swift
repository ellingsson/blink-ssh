...
open class TerminalView: UIScrollView, UITextInputTraits, UIKeyInput, UIScrollViewDelegate, TerminalDelegate {
    ...
    private var mousePanUsesWheel = false
    private var mousePanHasSentPress = false
    private var directSelectionMode = false

    public func setDirectSelectionMode(_ enabled: Bool) {
        directSelectionMode = enabled
        allowMouseReporting = !enabled
    ...
    @objc func panMouseHandler (_ gestureRecognizer: UIPanGestureRecognizer){
        ...
        if allowMouseReporting && terminal.mouseMode != .off {
            ...
            case .began:
                mousePanUsesWheel = false
                mousePanHasSentPress = false
            case .changed:
                let translation = gestureRecognizer.translation(in: self)
                if !mousePanHasSentPress && abs(translation.y) > abs(translation.x) {
                    let wheelEvents = MouseWheelGesture.eventCount(for: translation.y, pointsPerEvent: 24)
                    guard wheelEvents != 0 else { return }
                    mousePanUsesWheel = true
                    sendMouseWheel(events: wheelEvents, gestureRecognizer: gestureRecognizer)
                    gestureRecognizer.setTranslation(.zero, in: self)
                } else if !mousePanUsesWheel {
                    if !mousePanHasSentPress, terminal.mouseMode.sendButtonPress() {
                        sharedMouseEvent(gestureRecognizer: gestureRecognizer, release: false)
                        mousePanHasSentPress = true
                    }
            ...
            case .ended, .cancelled:
                if mousePanHasSentPress && terminal.mouseMode.sendButtonRelease() {
                    sharedMouseEvent(gestureRecognizer: gestureRecognizer, release: true)
                }
            default:
                break
    ...
    private func sendMouseWheel(events: Int, gestureRecognizer: UIGestureRecognizer) {
        let hit = calculateTapHit(gesture: gestureRecognizer)
        guard let grid = hit.grid.toScreenCoordinate(from: terminal.displayBuffer) else { return }
        let button = MouseWheelGesture.button(for: events)
        let flags = terminal.encodeButton(button: button, release: false, shift: false, meta: false, control: false)
        for _ in 0..<abs(events) {
            terminal.sendEvent(buttonFlags: flags, x: grid.col, y: grid.row, pixelX: hit.pixels.col, pixelY: hit.pixels.row)
        }
    }
        ...
        for press in presses {
            ...
            default:
                if key.modifierFlags.contains ([.alternate, .command]) && key.charactersIgnoringModifiers == "o" {
                    ...
                    // On non-US hardware keyboard layouts, Option can produce a
                    // printable character (for example ø) while
                    // charactersIgnoringModifiers refers to a different physical
                    // key (for example 8).  Send that character rather than
                    // turning it into Meta-8.
                    if key.characters.unicodeScalars.contains(where: { $0.value > 0x7f }) {
